...
if (isset($_POST['add_faculty'])) {
    ...
    $user_rights = $is_focal_person ? 'focal_person' : 'normal';

    // If setting as focal person, remove focal person status from other faculty in the same department
    if ($is_focal_person == 1) {
        $stmt = $conn->prepare("UPDATE faculty SET is_focal_person = 0, user_rights = 'normal' WHERE department_id = ? AND is_focal_person = 1");
        $stmt->bind_param("i", $department_id);
        $stmt->execute();
        $stmt->close();
    }

    $stmt = $conn->prepare("INSERT INTO faculty (first_name, last_name, email, phone, department_id, hire_date, bio, password, is_focal_person, user_rights) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssisssis", $first_name, $last_name, $email, $phone, $department_id, $hire_date, $bio, $password, $is_focal_person, $user_rights);
    $stmt->execute();
    $stmt->close();
...
if (isset($_POST['update_faculty'])) {
    ...
    // If setting as focal person, remove focal person status from other faculty in the same department
    if ($is_focal_person == 1) {
        $stmt = $conn->prepare("UPDATE faculty SET is_focal_person = 0, user_rights = 'normal' WHERE department_id = ? AND id != ? AND is_focal_person = 1");
        $stmt->bind_param("ii", $department_id, $id);
        $stmt->execute();
        $stmt->close();
    }

    $stmt = $conn->prepare("UPDATE faculty SET first_name=?, last_name=?, email=?, phone=?, department_id=?, hire_date=?, bio=?, is_focal_person=?, user_rights=? WHERE id=?");
    $stmt->bind_param("ssssissssi", $first_name, $last_name, $email, $phone, $department_id, $hire_date, $bio, $is_focal_person, $user_rights, $id);
    $stmt->execute();
    $stmt->close();
    header("Location: admin_faculty.php");
    exit();
...
// Get faculty with department names
$faculty = $conn->query("SELECT f.*, d.name as dept_name FROM faculty f LEFT JOIN departments d ON f.department_id = d.depart_id ORDER BY f.id DESC");
?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <title>Faculty - Learning Management System</title>
...
<body>
    <header>
        <h2>👨‍🏫 Faculty Management - Learning Management System</h2>
        <div class="nav-links">
    ...
    <div class="container">
        ...
        <div class="table-container">
            ...
            <button class="btn btn-add" onclick="openAddModal()">+ Add Faculty</button>
                    ...
                    <?php if ($faculty && $faculty->num_rows > 0): ?>
                        <?php while ($row = $faculty->fetch_assoc()): ?>
                            <tr>
                                ...
                                <td><?= htmlspecialchars(ucfirst(str_replace('_', ' ', $row['user_rights'] ?? 'normal'))) ?></td>
                                <td><?= $row['is_focal_person'] ? '⭐ Yes' : 'No' ?></td>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <label style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
                    <input type="checkbox" name="is_focal_person" style="width: auto;">
                    <span>Mark as Focal Person (can manage news, notices, events)</span>
                </label>
                <div style="text-align:right;">
                    <button type="button" class="btn close-btn" onclick="closeModal('addModal')">Cancel</button>
                    <button type="submit" name="add_faculty" class="btn btn-add">Add</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal" id="editModal">
        <div class="modal-content">
            ...
            <form method="POST">
                ...
                <textarea name="bio" id="edit_bio" placeholder="Bio (optional)"></textarea>
                <label style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;">
                    <input type="checkbox" name="is_focal_person" id="edit_is_focal_person" style="width: auto;">
                    <span>Mark as Focal Person (can manage news, notices, events)</span>
                </label>
                <div style="text-align:right;">
                    <button type="button" class="btn close-btn" onclick="closeModal('editModal')">Cancel</button>
                    <button type="submit" name="update_faculty" class="btn btn-edit">Update</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        function openEditModal(faculty) {
            ...
            document.getElementById('edit_bio').value = faculty.bio || '';
            document.getElementById('edit_is_focal_person').checked = faculty.is_focal_person == 1;
            // Show warning if another focal person exists in the same department
            if (faculty.is_focal_person == 0) {
                // Could add AJAX check here if needed
            }
        }
